"""Vercel cron — same-day 1-hour reminder. Fires at 10 UTC (6 PM PHT) daily.

On webinar dates it pulls the unified recipient list (free-webinar regs ∪
Business Unlocked ticket buyers) and sends the "Starts in ~1 hour" reminder.
"""
...
def _send_one(template: str, w: dict, row: dict, api_key: str,
    ...
    html = render(template, {
        "name": first,
        "preheader": f"Starts soon — {w['speaker']} on {w['topic']}.",
        "urgency_label": "Starts today · Join on Zoom",
        "headline": "Starts in about 1 hour",
        "lead": (
            f"Your webinar with <strong>{w['speaker']}</strong> is starting soon. "
            f"Same Zoom link you've been using — tap below to join. "
        ...
        "speaker": w["speaker"],
        "topic": w["topic"],
        "time_label": f"Today · {w['date_label']}, 2026 · {w['time_hour_pht']}:00 PM PHT",
    ...
    status, body = send_brevo(
        ...
        f"Starts in 1 hour — {w['speaker']} · W{w['num']}", html,
    )
    ...
    if force_num:
        ...
        if not w:
            return {"ok": True, "skipped": True,
                    "reason": f"no webinar on {_pht_today().isoformat()} (PHT)"}
